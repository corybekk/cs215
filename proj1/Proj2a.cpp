...
public:
  sentence();
  sentence(const sentence& other_word);
  ~sentence():
...
sentence::sentence(const sentence& other_word){
}
sentence::~sentence(){
}*/
//-------------------------EMPTY LINE FUNCTION--------------------------------------
...
bool check_Vocab(string SENTENCE){
  ...
  //--------SUBJECT AND SUBJECT---------
  while(check_and(array4,i)!="..."){//keeps looping as long as there is "and"
  if(check_and(array4,i)!="..."){
    if(i==w-1){
  ...
  if(check_verb(array4,i)!="..."){ //checks if current index spot is a verb.
	...
	print_sentence(current_word,w,array4);
	return false;
      }      
  ...
  //--------VERB AND VERB----------------
  while(check_and(array4,i)!="..."){
    if(check_and(array4,i)!="..."){
      if(i==w-1){
	cout<<"Error: invalid predicate_5";
	return false;
      }
      i++;
      if(check_verb(array4,i)!="..."){
...
string check_and(string Array8[], int x9){
  ...
  int i=0;
  i=x9;
  if(Array8[i]== "and") return Array8[i];
  return "...";
}
//--------------------CHECK PREPOSITION--------------------------
